    ...
    <>
          ...
          <div className='grid grid flow-col justify-items-center sm:justify-items-start grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 p-4 gap-x-32 gap-y-4'>
            {tableData.map((d) => (
              <span className='relative group hover:z-20 flex flex-row items-center w-60 h-10 rounded-md shadow-lg text-base text-black bg-orange-400 leading-5'>
                <CheckCircle className='text-white ml-1 mr-2' fontSize='default' />{d}
              </span>
            ))}
          </div>
        </div>
        <div className='relative right-0 h-48 w-full md:w-64 md:h-64 md:mr-24'>
          <Image src='/images/princeton.png' className='rounded-2xl' layout='fill' objectFit='contain' />
        </div>
      </div>
      ...
      <div className='flex flex-col mt-32 mb-16 md:flex-row-reverse md:justify-between items-center h-auto lg:h-96 w-full bg-orange-500 pb-3'>
        <div className='relative translate-y-3 w-64 h-32 md:translate-y-0 md:w-64 md:h-96 md:mr-16'>
          <Image src='/images/tutortechy.png' className='rounded-2xl' layout='fill' objectFit='contain' />
        </div>
      ...
      <div >
        <h1 className='text-center text-3xl md:text-5xl text-orange-500 font-bold pb-16 mx-3'>See my personal projects over on the <Link href='/projects'><a className='hover:text-blue-700 underline transition-color duration-300'>Projects page!</a></Link></h1>
      </div>

      <Link href='/'><a><p className='text-center hover:text-blue-500 hover:underline'>Back to home</p></a></Link>
      <br />
    </>
  )
}
